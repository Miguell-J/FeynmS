...
class QuantumCircuit:
    ...
    def execute(self, shots: int = 1) -> Dict[str, int]:
        ...
        for _ in range(shots):
            current_state = copy.deepcopy(self.qreg)
            measured_bits = self.creg.bits.copy()
            for op in self.operations:
                if op.gate.name == "Measure":
                    qubit_idx = op.qubits[0]
                    cbit_idx = op.classical_bits[0]
                    result, _ = current_state[qubit_idx].measure()
                    measured_bits[cbit_idx] = result
                else:
                    if len(op.qubits) == 1:
                        current_state[op.qubits[0]] = op.gate.apply(current_state[op.qubits[0]])
                    else:
                        qubits_involved = [current_state[q] for q in op.qubits]
                        new_state = op.gate.apply(qubits_involved)
                        if isinstance(new_state, MultiQubitState):
                            for i, q_idx in enumerate(op.qubits):
                                current_state[q_idx] = new_state
